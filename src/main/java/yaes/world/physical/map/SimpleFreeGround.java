...
/**
 * This class implements a map which represents a certain geographical area
 * where every point is either an inaccessible obstacle, or an area which can be
 * accessed.
 * 
 * @author Lotzi Boloni
 */
public class SimpleFreeGround implements IMap {
	...
	public boolean getPropertyAtAsBoolean(String propertyName, Location location) {
		final Boolean value = (Boolean) getPropertyAt(propertyName, location.getX(), location.getY());
	...
	private void loadBackGroundImage(String imageFileName) {
		...
		File imageFile = null;
		try {
			imageFile = new File(ClassLoader.getSystemClassLoader().getResource(imageFileName).toURI());
		} catch (URISyntaxException e1) {
			// TODO Auto-generated catch block
			e1.printStackTrace();
		}
		if (!imageFile.exists()) {
			TextUi.errorPrint("Image file:" + imageFileName + " does not exists");
	...
	/**
	 * This function initializes the internal accessibility structures from a
	 * map.
	 * 
	 * @param imageFileName
	 */
	 ...
	 */
	@Override
	public void setPropertyAt(String propertyName, double x, double y, Object value) {
